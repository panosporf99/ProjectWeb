...
if ($_SERVER["REQUEST_METHOD"]==="POST"){
	...
	if(mysqli_num_rows($rslt) != 0){			
			...
			echo "There is a username or email already registered. Change that USERNAME OR EMAIL!!!";
	}elseif($password_1 != $password_2){
		alertBox("Passwords must match brooo!!!");
		echo "Passwords must match brooo!!!";
		...
		echo "Passwords must contain at least one number, one symbol, one capital letter and be more than 8 characters";
	}else{
		
		
		echo "Gamw ton Xristo";
		...
		mysqli_query($db,$result);//gia na ginei to insert
